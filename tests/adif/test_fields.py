...
def test_headers_and_qsos_w_tipes():
    imput = Position('<PROGRAMID:9>ADIF2JSON\n<PROGRAMVERSION:5>0.0.1\n<EOH>\n<QSO_DATE:8:S>20180101\n<EOR>\n')
    res = _read_fields(imput)

    assert res == Adif(headers=Record({'PROGRAMID': 'ADIF2JSON', 'PROGRAMVERSION': '0.0.1'}), qsos=[Record({'QSO_DATE': '20180101'}, {'QSO_DATE': 'S'})], errors=None)


def test_headers_and_qsos_compact():
    imput = Position('<PROGRAMID:9>ADIF2JSON<PROGRAMVERSION:5>0.0.1<EOH><QSO_DATE:8>20180101<EOR>')
    res = _read_fields(imput)
...
def test_headers_and_qsos_bad_size():
    imput = Position('<PROGRAMID:9>ADIF2JSON<PROGRAMVERSION:5>0.0.1<EOH><CALL:x>EA4HFF<QSO_DATE:8>20180101<EOR>')
    res = _read_fields(imput)

    assert res == Adif(headers=Record({'PROGRAMID': 'ADIF2JSON', 'PROGRAMVERSION': '0.0.1'}), qsos=[Record({'QSO_DATE': '20180101'})], errors=[Reason.INVALID_SIZE])
...
def test_label_but_value():
    ...
    assert res == Adif(headers=None, qsos=None, errors=[Reason.TRUNCATED_FILE]), res
...
def test_no_size_label():
    ...
    assert res == Adif(headers=None, qsos=None, errors=[Reason.INVALID_SIZE]), res
